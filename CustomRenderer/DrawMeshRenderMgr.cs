    ...
    public class DrawMeshRenderMgr
    {
        ...
        public class DrawMeshBatchNode
        {
            ...
            public void ImportData(int index, int count, Matrix4x4[] matrix4X4s, MaterialPropertyBlock propertyBlock)
            {
                ...
                if (lightmapIndex.Count > 0 )
                {

                    LightmapMgr.TexturePackage texturePackage_Morning = null;//LightmapMgr.Inst.GetTexturePackageByInfo(lightmapType, lightmapIndex[0] ,LightmapMgr.LightmapType.Morning);
                    LightmapMgr.TexturePackage texturePackage_Evening = null;// = LightmapMgr.Inst.GetTexturePackageByInfo(lightmapType, lightmapIndex[1], LightmapMgr.LightmapType.Evening);

                    for (int i = 0; i < lightmapIndex.Count; i++)
                    {
                        if (i == 0)
                             texturePackage_Morning = LightmapMgr.Inst.GetTexturePackageByInfo(lightmapType, lightmapIndex[i], LightmapMgr.LightmapType.Morning);
                        if(i == 1)
                             texturePackage_Evening = LightmapMgr.Inst.GetTexturePackageByInfo(lightmapType, lightmapIndex[i], LightmapMgr.LightmapType.Evening);
                    }

                    if (texturePackage_Morning != null || texturePackage_Evening != null )
                    {
                        List<Vector4> listmapST_Morning = tempV4List;
                        List<Vector4> listmapST_Evening = new List<Vector4>();
                        tempV4List.Clear();
                        for (int i = 0; i < count; i++)
                        {
                            DrawMeshInfo info = infos[index + i];
                            for (int j = 0; j < info.lightmapProp.Count; j++)
                            {
                                if( j == 0)
                                    listmapST_Morning.Add(info.lightmapProp[j].lightmapST);
                                if(j == 1)
                                    listmapST_Evening.Add(info.lightmapProp[j].lightmapST);
                            }
                         //   listmapST_Morning.Add(info.lightmapProp[0].lightmapST);
                         //   listmapST_Evening.Add(info.lightmapProp[1].lightmapST);
                        }
                        
                        if (texturePackage_Morning.lightmapColor != null)
                            propertyBlock.SetTexture(LightmapNode.Lightmap_Morning, texturePackage_Morning.lightmapColor);

                        if (texturePackage_Morning.lightmapDir != null)
                            propertyBlock.SetTexture(LightmapNode.LightmapInd_Morning, texturePackage_Morning.lightmapDir);

                        if (texturePackage_Morning.shadowMask != null)
                            propertyBlock.SetTexture(LightmapNode.ShadowMask_Morning, texturePackage_Morning.shadowMask);


                        if (texturePackage_Evening.lightmapColor != null)
                            propertyBlock.SetTexture(LightmapNode.Lightmap_Evening, texturePackage_Evening.lightmapColor);
                        if (texturePackage_Evening.lightmapDir != null)
                            propertyBlock.SetTexture(LightmapNode.LightmapInd_Evening, texturePackage_Evening.lightmapDir);
                        if (texturePackage_Evening.shadowMask != null)
                            propertyBlock.SetTexture(LightmapNode.ShadowMask_Evening, texturePackage_Evening.shadowMask);


                        propertyBlock.SetVectorArray(LightmapNode.Custom_LightmapST_Morning_ID, listmapST_Morning);
                        propertyBlock.SetVectorArray(LightmapNode.Custom_LightmapST_Evening_ID, listmapST_Evening);
